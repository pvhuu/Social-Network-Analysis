...
def usage():
    print("Usage:")
    print("py {} <filename.jsonl>".format(sys.argv[0]))
...
if __name__=='__main__':
    ...
    with open(fname,'r') as f:
        ...
        print("{} tweets with at least one hashtags ({}%)".format(tweets_with_hashtags,tweets_with_hashtags_percent))
            
        for tag_count, tweet_count  in hashtag_count.items():
            if(tag_count>0):
                percent_total = "%.2f" % (tweet_count / tweets_total * 100)
                percent_elite = "%2.f" % (tweet_count / tweets_with_hashtags * 100)
